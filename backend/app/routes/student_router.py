...
@router.post("/students/add/", response_model=MutationResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(RoleRequired(3))])
...
@router.get("/students/get/{student_id}", response_model=StudentResponse, dependencies=[Depends(verify_auth)])
...
@router.put("/students/update/{student_id}", response_model=MutationResponse, dependencies=[Depends(RoleRequired(3))])
async def update_student(
    ...
    return service.update(student_id, updates.model_dump(exclude_unset=True))

@router.delete("/students/delete/{student_id}", response_model=MutationResponse, dependencies=[Depends(RoleRequired(3))])
...
@router.get("/students/get/", response_model=List[StudentResponse], dependencies=[Depends(verify_auth)])
