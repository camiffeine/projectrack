...
@router.post("/student_assignments/add/", status_code=status.HTTP_201_CREATED, dependencies=[Depends(RoleRequired(2, 3))])
...
@router.get("/student_assignments/get/{assignment_id}", dependencies=[Depends(verify_auth)])
...
@router.put("/student_assignments/update/{assignment_id}", dependencies=[Depends(RoleRequired(2, 3))])
...
@router.delete("/student_assignments/delete/{assignment_id}", dependencies=[Depends(RoleRequired(2, 3))])
...
@router.get("/student_assignments/get/", dependencies=[Depends(verify_auth)])
@router.get("/student_assignment", dependencies=[Depends(verify_auth)], include_in_schema=False)
